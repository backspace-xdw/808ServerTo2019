...
> ⚠️ **历史快照, 已与当前源码不一致, 请勿作为开发参考**
>
> 本文件是 **2025-11-24** 生成的代码快照, 反映当时的初版实现。
>
> 此后项目经历了 **高并发优化重构** 和 **实时位置数据 XML 存储 + 归档双写** 等多次大改动:
> - `JT808TcpServer.cs` / `SessionManager.cs` / `LocationDataStore.cs` 已被全面重写
> - 新增了 `ServerConfig.cs` / `MediaDataStore.cs` / `appsettings.json`
> - 修复了若干 bug (接收 buffer offset 累加、并发 Send 错乱、format string 等)
>
> **以 `JT808.Server/` `JT808.Protocol/` 目录下的实际源码为准。**
> 当前最新结构和说明见 [`README.md`](./README.md) 和 [`项目说明.md`](./项目说明.md)。
